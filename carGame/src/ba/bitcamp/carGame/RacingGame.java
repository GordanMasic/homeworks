...
public class RacingGame extends JPanel implements ActionListener {
	...
	private static final long serialVersionUID = 1915513749312202916L;
	...
	public RacingGame(int roadWidth, int roadHeight, int speed) {
		...
		this.speed = speed;
		this.minX = Road.ROAD_SIDE;
		this.maxX = roadWidth - Road.ROAD_SIDE - Vehicle.WIDTH;

		this.myVehicle = new Vehicle(400, 500);
		Road road = new Road(roadWidth, roadHeight, speed);
		AnimatedVehicle otherVehicle = new AnimatedVehicle(minX, maxX,
				roadHeight, speed);
		OilStain oilStain = new OilStain(minX, maxX, roadHeight, speed);

		gameArtifacts = new GameArtifact[] { road, otherVehicle, oilStain,
				myVehicle };
		animatedGameArtifacts = new AnimatedGameArtifact[] { road,
				otherVehicle, oilStain };
		animatedVehicles = new AnimatedVehicle[] { otherVehicle };

		oilStains = new OilStain[] { oilStain };
	...
	public void actionPerformed(ActionEvent e) {
		...
		for (OilStain os : oilStains) {
			if (myVehicle.oilColade(os)) {
				myVehicle.crash();
				os.resetPosition();
			}
		}
